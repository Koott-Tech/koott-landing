...
const frequencies = ['individual', 'package'];
...
export function PricingSection({
	...
	const [frequency, setFrequency] = React.useState('individual');
...
export function PricingCard({
	...
	return (
		<div
			key={plan.name}
			className={cn(
				'relative flex w-full flex-col rounded-lg border bg-white',
				className,
			)}
			{...props}
		>
			...
			>
				<div className="absolute top-2 right-2 z-10 flex items-center gap-2">
					...
					{frequency === 'package' && (
						<p className="bg-primary text-primary-foreground flex items-center gap-1 rounded-md border px-2 py-0.5 text-xs">
							{Math.round(
								((plan.price.individual * 12 - plan.price.package) /
				...
				<h3 className="mt-2 flex items-end gap-1">
					...
					<span className="text-muted-foreground">
						{plan.name !== 'Free'
							? '/' + (frequency === 'individual' ? 'session' : 'package')
							: ''}
					</span>
				</h3>
			</div>
